...
import JsxDiary from "features/diary/components/JsxDiary"
...
export default function DiaryToday() {
  ...
  window.localStorage.setItem("sessionDiaryDate", dateFormat(new Date()))
  ...
  useEffect(() => {
    initManuscript(),
      ...
      dispatch(diaryFindRequest({
        user_id: 1,
        diary_date: dateFormat(testDate1)
      }))
  }, []);
